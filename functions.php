...
function init_mts_custom_files() {
    /**
     * MyTinySecrets LoveSchool Helpers
     */
    require_once 'includes/classes/mts_helpers.php';

    /**
     * Course Progress Bar
     */
    require_once 'includes/course-lessons-mts.php';

    add_action('left_sidebar_mts', 'get_progress_bar_and_metadata', 10);
    add_action('left_sidebar_mts', 'get_course_lessons', 20);

    /**
     * Custom Course metadata info
     */
    require_once 'includes/course-metadata.php';

    add_action('course_metadata', 'get_course_custom_data', 10);
    add_action('course_metadata', 'course_single_meta', 20);

    /**
     * Custom Sidebar Friends
     */
    require_once 'includes/ajax-friends-tabs-custom.php';

    /**
     * MyTinySecrets Admin Dashboard
     */
    require_once 'includes/mts-admin-dashboard.php';
    $mts_admin_dashboard = new mts_admin_dashboard();

    /**
     * Ajax Calls
     */
    require_once 'includes/classes/mts_ajax_calls.php';
    require_once 'includes/classes/mts_profile_ajax.php';
    require_once 'includes/classes/mts_comments_ajax.php';

    $mts_ajax_calls    = new mts_ajax_calls();
    $mts_profile_ajax  = new mts_profile_ajax();
    $mts_comments_ajax = new mts_comments_ajax();

    $mts_profile_ajax->check_user_special_todo();

    /**
     * Modal Quiz AJAX
     * Print HTML Questions
     */
    require_once 'includes/classes/mts_quiz_modal_ajax.php';
    require_once 'includes/classes/mts_questions_print.php';

    $mts_quiz_modal_ajax = new mts_quiz_modal_ajax();
}

add_action( 'init', 'init_mts_custom_files', 80 );

/**
 * Lets check if the user have all the necessary meta data
 * @author: Alejandro Orta
 */
function add_user_metadata() {
	if( !get_user_meta( get_current_user_id(), 'user_social_links', true ) ) {
		add_user_meta( get_current_user_id(), 'user_social_links', array( 
			'facebook'    => '',
			'twitter'     => '',
			'google-plus' => '',
			'youtube'     => '',
...
add_action( 'init', 'add_user_metadata' );
...
function boss_child_enqueue_styles() {
    ...
    wp_enqueue_script( 'child-js-custom', get_stylesheet_directory_uri() . '/js/functions.js', array('jquery-form'), '1.4.2', false );
    ...
    wp_enqueue_style( 'child-style-custom', get_stylesheet_directory_uri() . '/css/style.css', false, '1.4.2', 'all' );
...
$args = array(
    'name'          => __( 'Sidebar left for lessons', 'theme_text_domain' ),
    'id'            => 'sidebar-lessons',
    'description'   => 'Sidebar left for lessons',
    'class'         => '',
    'before_widget' => '',
    'after_widget'  => '',
    'before_title'  => '',
...
//add left sidebar for comunity
$args = array(
    'name'          => __( 'Sidebar left for Comunity', 'theme_text_domain' ),
    'id'            => 'sidebar-comunity',
    'description'   => 'Left sidebar for comunity',
    'class'         => '',
    'before_widget' => '',
    'after_widget'  => '',
    'before_title'  => '',
...
//add left sidebar for profile
$args = array(
    'name'          => __( 'Sidebar left for Profile', 'theme_text_domain' ),
    'id'            => 'sidebar-profile',
    'description'   => 'Left sidebar for profile',
    'class'         => '',
    'before_widget' => '',
    'after_widget'  => '',
    'before_title'  => '',
...
/**
 * Redirect login page to /login/ when visiting directly (not for login or logout actions)
 * 
 * @author Rafa Poveda
 */
function redirect_login_page() {
	...
	if( (strpos( $page_viewed, "wp-login.php?action=logout") !== false ) ) {

	}
	else {
		if( (strpos( $page_viewed, "wp-login.php") !== false )  && $_SERVER['REQUEST_METHOD'] == 'GET') {
			// And away they go...
